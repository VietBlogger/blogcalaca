...
class MikadoSkin extends WanderlandMikadoClassSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to wanderland_mikado_admin_scripts_init and wanderland_mikado_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'mikado';
		
		// Admin register scripts and styles
		add_action( 'wanderland_mikado_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'wanderland_mikado_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'wanderland_mikado_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'wanderland_mikado_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'wanderland_mikado_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'wanderland_mikado_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'wanderland_mikado_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			wanderland_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            wanderland_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= wanderland_mikado_get_admin_tab();
        $active_page 	= $wanderland_mikado_global_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see WanderlandMikadoClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param WanderlandMikadoClassAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(wanderland_mikado_get_theme_info_item('Name'), wanderland_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
