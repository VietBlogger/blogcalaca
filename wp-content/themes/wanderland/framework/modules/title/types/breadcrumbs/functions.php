...
if ( ! function_exists( 'wanderland_mikado_set_title_breadcrumbs_type_for_options' ) ) {
	/**
	 * This function set breadcrumbs title type value for title options map and meta boxes
	 */
	function wanderland_mikado_set_title_breadcrumbs_type_for_options( $type ) {
		$type['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'wanderland' );
		
		return $type;
	}
	
	add_filter( 'wanderland_mikado_filter_title_type_global_option', 'wanderland_mikado_set_title_breadcrumbs_type_for_options' );
	add_filter( 'wanderland_mikado_filter_title_type_meta_boxes', 'wanderland_mikado_set_title_breadcrumbs_type_for_options' );
}

if ( ! function_exists( 'wanderland_mikado_custom_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs
	 */
	function wanderland_mikado_custom_breadcrumbs() {
		global $post;
		
		$pageid    = wanderland_mikado_get_page_id();
		$homeLink  = esc_url( home_url( '/' ) );
		$home      = esc_html__( 'Home', 'wanderland' );
		...
		$output = '<div itemprop="breadcrumb" class="mkdf-breadcrumbs ' . esc_attr( $holder_classes ) . '" ' . wanderland_mikado_get_inline_attr( $holder_styles, 'style', ';' ) . '>';
			...
			if ( is_home() && ! is_front_page() ) {
				...
				if ( is_tag() ) {
					$childContent .= $before . esc_html__( 'Posts tagged ', 'wanderland' ) . '"' . single_tag_title( '', false ) . '"' . $after;
					...
					$childContent    .= $before . esc_html__( 'Articles posted by ', 'wanderland' ) . get_the_author_meta( 'display_name', $author_id ) . $after;
					...
					$childContent .= $before . esc_html__( 'Search results for ', 'wanderland' ) . '"' . get_search_query() . '"' . $after;
					
				} elseif ( is_404() ) {
					$childContent .= $before . esc_html__( 'Error 404', 'wanderland' ) . $after;
				...
				if ( get_query_var( 'paged' ) ) {
					$childContent .= $before . " (" . esc_html__( 'Page', 'wanderland' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
					
				}
				
				$output .= apply_filters( 'wanderland_mikado_filter_breadcrumbs_title_child_output', $childContent, $delimiter, $before, $after );
			}
		
		$output .= '</div>';
		
		echo wp_kses( apply_filters( 'wanderland_mikado_filter_breadcrumbs_title_output', $output ), array(
			'div'  => array(
				'itemprop' => true,
				'id'       => true,
				'class'    => true,
