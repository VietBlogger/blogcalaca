...
namespace WanderlandMikadoNamespace\Modules\Header\Lib;

/**
 * Class WanderlandMikadoClassHeaderConnector
 *
...
class WanderlandMikadoClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'wanderland_mikado_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'wanderland_mikado_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'wanderland_mikado_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'wanderland_mikado_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to wanderland_mikado_filter_title_content_padding filter
	 *
	 * @return int
	 */
