 ...
 */
if ( ! class_exists( 'WanderlandMikadoClassSidebar' ) ) {
	class WanderlandMikadoClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'wanderland' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'wanderland-mikado-admin-sidebar', WANDERLAND_MIKADO_ROOT . '/framework/admin/assets/js/mkdf-ui/mkdf-sidebar.js' );
			wp_enqueue_style( 'wanderland-mikado-admin-sidebar', WANDERLAND_MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'wanderland') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'wanderland') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ( $key = array_search( $name, $this->sidebars ) ) !== false ) {
					...
					update_option( $this->stored, $this->sidebars );
					echo esc_html__( "sidebar-deleted", 'wanderland' );
				}
			}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'wanderland_mikado_filter_custom_widget_args', $args );
