...
include WANDERLAND_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( WANDERLAND_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( wanderland_mikado_is_customizer_item_enabled( $module_load, 'wanderland_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'wanderland_mikado_get_icon_pack_collections' ) ) {
	function wanderland_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'wanderland_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'wanderland_mikado_get_icon_pack_options' ) ) {
	function wanderland_mikado_get_icon_pack_options() {
		$options = apply_filters( 'wanderland_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'wanderland_mikado_get_icon_pack_social_options' ) ) {
	function wanderland_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'wanderland_mikado_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: WanderlandMikadoClassIconCollections
 */
class WanderlandMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = wanderland_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = wanderland_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = wanderland_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = wanderland_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'wanderland' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'wanderland' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'wanderland' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'wanderland' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'wanderland' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'wanderland' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'wanderland' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'wanderland' )       => 'fa-lg',
			esc_html__( 'Small', 'wanderland' )      => 'fa-2x',
			esc_html__( 'Medium', 'wanderland' )     => 'fa-3x',
			esc_html__( 'Large', 'wanderland' )      => 'fa-4x',
			esc_html__( 'Very Large', 'wanderland' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iWanderlandMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'wanderland' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'wanderland' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'wanderland-mikado-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
