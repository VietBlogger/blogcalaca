...
/*
   Class: WanderlandMikadoClassMultipleImages
   A class that initializes Mikado Multiple Images
*/

class WanderlandMikadoClassMultipleImages implements iWanderlandMikadoInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$wanderland_mikado_global_Framework->mkdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="mkdf-gallery-uploader">
								<a class="mkdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'wanderland' ); ?></a>
								<a class="mkdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'wanderland' ); ?></a>
							</div>
...
class WanderlandMikadoClassTwitterFramework implements iWanderlandMikadoInterfaceRender {
	public function render( $factory ) {
		$twitterApi = WanderlandTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'wanderland' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'wanderland' ) : esc_html__( 'Connect with Twitter', 'wanderland' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'wanderland' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'wanderland' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class WanderlandMikadoClassInstagramFramework implements iWanderlandMikadoInterfaceRender {
	public function render( $factory ) {
		$instagram_api = WanderlandInstagramApi::getInstance();
		$message       = '';
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'wanderland' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'wanderland' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'wanderland' );
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'wanderland' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'wanderland' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ($instagram_user_id) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'wanderland' ); echo esc_attr($instagram_user_id) ?></p>
							</div>
						</div>
					<?php } ?>
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'wanderland' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'wanderland' ); ?></a>
							</div>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'wanderland' ); ?></a>
							</div>
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'wanderland' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class WanderlandMikadoClassRepeater implements iWanderlandMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'wanderland' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-repeater-table-heading">
					<div class="mkdf-repeater-fields-holder">
						<div class="mkdf-repeater-table-cell mkdf-repeater-sort"><?php esc_html_e( 'Order', 'wanderland' ) ?></div>
						...
						<div class="mkdf-repeater-table-cell mkdf-repeater-remove"><?php esc_html_e( 'Remove', 'wanderland' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="mkdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="mkdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = wanderland_mikado_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class WanderlandMikadoClassFieldAddress extends WanderlandMikadoClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Wanderland Options', 'wanderland' ); ?></p>
							<?php } ?>
							<a class="mkdf-reset-marker mkdf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'wanderland' ); ?></a>
...
class WanderlandMikadoClassFieldIcon extends WanderlandMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = wanderland_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = wanderland_mikado_icon_collections()->getIconCollectionsKeys();
		?>
			...
			<div class="mkdf-section-content">
				<div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = wanderland_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						...
						<div class="row mkdf-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
							<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control mkdf-form-element">
									<?php
									$icons       = wanderland_mikado_icon_collections()->getIconCollection( $icons_collection );
