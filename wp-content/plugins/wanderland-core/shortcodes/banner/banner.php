<?php
namespace WanderlandCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'wanderland-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'wanderland-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'wanderland-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'wanderland-core' )   => 'mkdf-visible-on-hover',
								esc_html__( 'Visible on Default', 'wanderland-core' ) => 'mkdf-visible-on-default',
								esc_html__( 'Disabled', 'wanderland-core' )           => 'mkdf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'wanderland-core' ),
							'value'       => array(
								esc_html__( 'Default', 'wanderland-core' )  => 'default',
								esc_html__( 'Centered', 'wanderland-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'wanderland-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'wanderland-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
							'heading'    => esc_html__( 'Subtitle', 'wanderland-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'subtitle_tag',
							'heading'     => esc_html__( 'Subtitle Tag', 'wanderland-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'wanderland-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'     => esc_html__( 'Words with Light Font Weight', 'wanderland-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "light" font weight. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a light font weight, you would enter "1,3,4")', 'wanderland-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'wanderland-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'wanderland-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'wanderland-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = wanderland_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . wanderland_mikado_filter_px( $params['link_top_margin'] ) . 'px';
		}
