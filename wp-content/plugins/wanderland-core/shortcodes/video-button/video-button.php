...
namespace WanderlandCore\CPT\Shortcodes\VideoButton;
...
class VideoButton implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Video Button', 'wanderland-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Video Link', 'wanderland-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'video_image',
							'heading'     => esc_html__( 'Video Image', 'wanderland-core' ),
							'description' => esc_html__( 'Select image from media library', 'wanderland-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'play_button_color',
							'heading'    => esc_html__( 'Play Button Color', 'wanderland-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'play_button_size',
							'heading'    => esc_html__( 'Play Button Size (px)', 'wanderland-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'play_button_image',
							'heading'     => esc_html__( 'Play Button Custom Image', 'wanderland-core' ),
							'description' => esc_html__( 'Select image from media library. If you use this field then play button color and button size options will not work', 'wanderland-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'play_button_hover_image',
							'heading'     => esc_html__( 'Play Button Custom Hover Image', 'wanderland-core' ),
							'description' => esc_html__( 'Select image from media library. If you use this field then play button color and button size options will not work', 'wanderland-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = wanderland_core_get_shortcode_module_template_part( 'templates/video-button', 'video-button', '', $params );
	...
	private function getPlayButtonStyles( $params ) {
		...
		if ( ! empty( $params['play_button_size'] ) ) {
			$styles[] = 'font-size: ' . wanderland_mikado_filter_px( $params['play_button_size'] ) . 'px';
		}
