...
namespace WanderlandCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
					...
					'params'                    => array(
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Boxed', 'wanderland-core' )               => 'boxed',
								esc_html__( 'Image Pagination', 'wanderland-core' )    => 'image-pagination',
								esc_html__( 'Boxed Text', 'wanderland-core' )          => 'boxed-text',
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'wanderland-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
						...
						array(
							...
							'heading'    => esc_html__( 'Content Box Color', 'wanderland-core' ),
	...
	public function render( $atts, $content = null ) {
        ...
        return wanderland_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'wanderland-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'wanderland-core' ) . ': ' . $testimonials_category_title;
				}
