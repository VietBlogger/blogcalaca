    ...
    <div class="mkdf-tl-wrapper mkdf-outer-space">
            ...
            <?php if ( is_array( $response->data ) && count( $response->data ) ) { ?>
                <ul class="mkdf-twitter-list">
                    <?php foreach ( $response->data as $tweet ) { ?>
                        <?php
                            $params['tweet'] = $tweet;
                            echo wanderland_twitter_get_shortcode_module_template_part('templates/item', 'twitter-list', '', $params);
                        ?>
                    <?php } ?>
                </ul>
            <?php } else { ?>
        ...
        <?php } else { ?>
            <div class="mkdf-twitter-not-connected">
                <?php esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'wanderland-twitter-feed' ); ?>
            </div>
        <?php } ?>
    </div>
</div>